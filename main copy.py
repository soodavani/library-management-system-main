...
#change name
def Submit(id_entry, bookname_entry, author_entry, edition_entry, price_entry, qty_entry): #addnewbookfunction 
...
def IssueBookGUI():
    ...
    return idi_entry, booknamei_entry, name_entry, roll_entry, issue_entry, return_entry
def Submit_Issue(idi_entry, booknamei_entry, name_entry, roll_entry, issue_entry, return_entry): #addnewbookfunction
    if idi_entry == "" or booknamei_entry == "" or name_entry == "" or roll_entry == "" or issue_entry == "" or return_entry == "":
        messagebox.showerror("Error!", "Sorry, all fields are required", parent=root)
    else:
        try:
            curs.execute("select * from records where bookid=%i", (idi_entry,))
            row = curs.fetchone()
            book_quantity = row[5]
            if row != None:
                messagebox.showerror("Error!", "This book does not exist", parent=root)
            elif book_quantity == 0:
                    messagebox.showwarning("Notification", "There are no copies left")
            else:
                curs.execute("insert into records (bookid,bookname,studentroll,studentname,issuedt,returndt) values(%s,%s,%s,%s,%d,%d)",
                                (
                                    idi_entry,
                                    booknamei_entry,
                                    roll_entry,
                ...
                book_quantity-=1
                curs.execute("update books set qty=%s where book_id=%s", (book_quantity, idi_entry.get()))
                connection.commit()
                messagebox.showinfo('Done!', "The data has been submitted")
                # Call the reset_fields() method here if available
...
def ShowRecordsforReturn(roll_no, window, frame_lhs):
    if roll_no == "":
        ...
        try:
            ...
            curs = connection.cursor()
            curs.execute("select * from records where stu_roll=%s", roll_no)
            rows = curs.fetchall()
...
add_book = tk.Button(frame_rhs, text='Add Book', font=(cs.font_1, 12), bd=2, command=AddNewBookGUI, cursor="hand2", bg=cs.color_2, fg=cs.color_3)
add_book.place(x=180, y=100, width=100)

BookHolders_button = tk.Button(frame_rhs, text='Book Holders', font=("Arial", 12), bd=2, command=BookHoldersGUI_Func, cursor="hand2", bg="gray", fg="black")
...
issue_book_button = tk.Button(frame_rhs, text='Issue Book', font=("Arial", 12), bd=2, command=IssueBookGUI, cursor="hand2", bg="gray", fg="black")
issue_book_button.place(x=50, y=160, width=100)

search_book_button = tk.Button(frame_rhs, text='Search Book', font=("Arial", 12), bd=2, command=SearchGUI, cursor="hand2", bg="gray", fg="black") 
