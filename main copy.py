...
def IssueBookGUI():
    ...
    show_books = tk.Button(frame_lhs, text='Display all books', font=(cs.font_1, 12), bd=2, command=ShowBooksGUI, cursor="hand2", bg=cs.color_2, fg=cs.color_3).place(x=310, y=459, width=100)
...
def ShowRecordsforReturn(roll_no_entry, frame_lhs):
    if roll_no_entry == "":
...
def ReturningBook(tree_1, row, delete_button):
    try:
...
def OnSelectedforReturn(event): 
    try:
        ...
        if selection:
            ...
            delete_button = tk.Button(frame_3, text='Delete', font=(cs.font_1, 12), bd=2, command=lambda r=row: ReturningBook(tree_1, r, delete_button), cursor="hand2", bg=cs.color_2, fg=cs.color_3)
